...
const ACCENT = "#16a34a";
const STEP_LABELS = ["Drop metal", "Observe", "Result"];

export default function ChemGroup1Lab() {
  ...
  const metal = cfg?.metal || METALS[0];
  const [dropped, setDropped] = useState(false);
  const [status, setStatus] = useState("idle");
  const [message, setMessage] = useState("Drop the metal into the water.");
  ...
  function drop() {
    ...
    setDropped(true);
    animTo(bubble, 1, 800);
    if (metal.flame) animTo(flame, 1, 600);
    Animated.delay(900).start(() => {
      setStatus("success");
      setMessage(`✅ ${metal.name}: ${metal.vigor}.`);
    });
  }

  function reset() {
    setDropped(false); setStatus("idle"); setSaved(false); setNotes("");
    bubble.setValue(0); flame.setValue(0);
    setMessage("Drop the metal into the water.");
  }

  async function saveReport() {
    const report = {
      id: Date.now().toString(),
      experimentId: experiment.id || "group1-reactivity",
      experimentTitle: experiment.title || "Reactivity of Group 1 Metals",
      topicId,
      date: new Date().toLocaleDateString(),
      metal: metal.name,
      observation: metal.vigor,
      result: `Observed: ${metal.vigor}`,
      status: "success",
      summary: `Dropped ${metal.name} into water. ${metal.vigor}. Trend: ${REACT_ORDER}.`,
      notes: notes.trim(),
    };
  ...
  const step = dropped ? (status === "success" ? 3 : 1) : 0;
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.bg }} edges={["top", "left", "right"]}>
      ...
      <View style={{ flexDirection: "row", alignItems: "center", paddingHorizontal: 16, paddingVertical: 12, backgroundColor: theme.colors.surface, borderBottomWidth: 1, borderBottomColor: theme.colors.border }}>
        ...
        <Text style={{ flex: 1, textAlign: "center", fontSize: 16, fontWeight: "800", color: theme.colors.text, marginRight: 40 }} numberOfLines={1}>{experiment.title || "Group 1 Reactivity"}</Text>
      </View>
      ...
      <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
        ...
        <Animated.View style={{ position: "absolute", top: 30, opacity: bubble }}>
          {[0, 1, 2].map((i) => (
            <Svg key={i} width={20} height={20} viewBox="0 0 20 20" style={{ marginBottom: 2 }}>
              <Circle cx={10} cy={10} r={5} fill="#ffffff" opacity={0.8} />
            </Svg>
          ))}
        </Animated.View>
        <Animated.View style={{ position: "absolute", top: 10, opacity: flame }}>
          <Svg width={60} height={60} viewBox="0 0 60 60">
            <Circle cx={30} cy={30} r={26} fill={metal.flame || "#c084fc"} opacity={0.85} />
            <Circle cx={30} cy={30} r={14} fill="#fff" opacity={0.5} />
          </Svg>
        </Animated.View>
        <Text style={labelStyle}>{dropped ? (metal.flame ? "Ignited!" : "Reacting") : "Water in beaker"}</Text>
      </View>
      ...
      <View style={{ backgroundColor: theme.colors.surface, borderTopLeftRadius: 20, borderTopRightRadius: 20, paddingHorizontal: 16, paddingTop: 14, paddingBottom: 18, shadowColor: "#000", shadowOffset: { width: 0, height: -3 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 10 }}>
        ...
        {status === "success" ? (
          ...
          <Btn label="① Drop metal" onPress={drop} accent={ACCENT} full />
        )}
      </View>
        ...
        <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.45)", justifyContent: "flex-end" }}>
          <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ maxHeight: "88%" }}>
            <ScrollView style={{ maxHeight: "100%" }} contentContainerStyle={{ backgroundColor: theme.colors.surface, borderTopLeftRadius: 22, borderTopRightRadius: 22, padding: 24, paddingBottom: 32 }} keyboardShouldPersistTaps="handled">
              ...
              <Text style={{ fontSize: 18, fontWeight: "800", textAlign: "center", color: theme.colors.text, marginTop: 6 }}>Reaction Recorded!</Text>
              <View style={{ backgroundColor: theme.colors.surfaceMuted, borderRadius: 12, padding: 14, marginVertical: 14 }}>
                <Text style={{ fontSize: 13, color: theme.colors.textMuted }}>• Metal: <Text style={{ fontWeight: "800" }}>{metal.name}</Text></Text>
                <Text style={{ fontSize: 13, color: theme.colors.textMuted, marginTop: 4 }}>• Observation: {metal.vigor}</Text>
                <Text style={{ fontSize: 12, color: "#94a3b8", marginTop: 6 }}>• Trend: {REACT_ORDER}</Text>
              </View>
              ...
              <TextInput
                ...
                placeholder="e.g. The sodium darted across the surface and melted into a glowing ball."
                value={notes}
