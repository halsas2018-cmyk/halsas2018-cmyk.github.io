...
export default function PhysMomentsPrep() {
  ...
  function start() {
    haptic.light();
    navigation.navigate("PhysMomentsLab", { experiment, topicId, config: {} });
  }
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.bg }} edges={["top", "left", "right"]}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={0}>
        <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 24 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <FlipCard emoji="⚖️" title="The principle of moments" front="For a body in equilibrium, the sum of clockwise moments equals the sum of anticlockwise moments." back="Moment of a force = force × perpendicular distance from the pivot (N·m, or g·cm in this sim)." />
          <FlipCard emoji="🔢" title="Balancing a beam" front="Fix a load on the left; choose a matching load + distance on the right so the beam is level." back="Anticlockwise moment (left) must equal clockwise moment (right): m_L·d_L = m_R·d_R." />
          <FlipCard emoji="🎯" title="Your task" front="Pick the right-hand mass and its distance to balance the left load." back="Next: (1) choose right mass, (2) choose its distance, (3) check the beam is level." />
        </ScrollView>
