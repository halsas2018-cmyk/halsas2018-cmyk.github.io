...
export default function PhysLcrResonancePrep() {
  ...
  function start() {
    haptic.light();
    navigation.navigate("PhysLcrResonanceLab", { experiment, topicId, config: {} });
  }
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.bg }} edges={["top", "left", "right"]}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={0}>
        <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 24 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <FlipCard emoji="📡" title="Resonance in LCR" front="In a series LCR circuit the current is largest when the inductive and capacitive reactances cancel." back="At resonance X_L = X_C, so Z = R (minimum) and the current I = V/R is maximum." />
          <FlipCard emoji="🎚️" title="Tuning the frequency" front="Vary the AC frequency; the current peaks at the resonant frequency f₀ = 1 / (2π√LC)." back="Below or above f₀ the reactance grows and the current falls." />
          <FlipCard emoji="🎯" title="Your task" front="Sweep the frequency to find where the current is maximum." back="Next: (1) pick a frequency, (2) read the ammeter, (3) tune to resonance." />
        </ScrollView>
